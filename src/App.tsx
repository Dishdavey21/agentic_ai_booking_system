...
type TripInput = {
  ...
  interests: string[]; // e.g. ["Food", "History"]
};
...
/* ---------------- MOCK DATA (STUDENT-FRIENDLY, NO APIS) ---------------- */
...
const POIS: POI[] = [
  ...
  { id: "m4", city: "Mumbai", name: "Chhatrapati Shivaji Maharaj Terminus (CSMT)", tags: ["History", "Architecture", "Photography"], estCost: "Free", bestTime: "Afternoon", durationHrs: 1.5 },
...
/* ---------------- ITINERARY AGENTS (REAL “AGENTIC” VALUE) ---------------- */
...
function interestsAgent(input: TripInput): { tags: string[]; trace: string } {
  ...
  return {
    tags,
    trace: `InterestsAgent set target tags = ${tags.join(", ")}.`,
  };
}

function budgetPolicyAgent(input: TripInput): { maxPaidPerDay: number; trace: string } {
  // Simple budget policy: how many “paid” items per day we allow
  // Shoestring: mostly free/low; Value: mix; Comfort: allow more medium
...
function schedulerAgent(input: TripInput, pois: POI[], maxPaidPerDay: number): { itinerary: DayPlan[]; trace: string[] } {
  const trace: string[] = [];
  const days = Math.max(1, Math.min(7, input.days)); // cap for demo quality
    ...
    <div className="container">
      <div className="topbar">
        <div className="brand">
          ...
          <div>Agentic AI Travel Studio</div>
          <span className="badge">Student-friendly • No paid APIs</span>
        </div>
            ...
            <>
              <div className="cardHeader">
                <div>
                  ...
                  <div className="subtitle">Orchestrator ranks options using price/time/stops and explains the decision.</div>
                </div>
                <span className="pill">Human approval gate</span>
              </div>
              ...
              <div className="kpiRow">
                ...
                <span className="kpi">Analyst scoring + trace</span>
                <span className="kpi">Redirect simulated</span>
              </div>
              ...
              <div className="small">
                Note: Flight prices here are mock to demonstrate agent orchestration. Real-time pricing requires commercial APIs/partnerships.
              </div>
            </>
          ) : (
            <>
              <div className="cardHeader">
                <div>
                  ...
                  <div className="subtitle">Multi-agent planning: interests → POIs → schedule → budget constraints + trace.</div>
                </div>
              ...
              <div className="formRow" style={{ gridTemplateColumns: "1.2fr 0.8fr 1fr" }}>
                ...
                <select
                  ...
                  onChange={(e) => setTripInput({ ...tripInput, budgetStyle: e.target.value as TripInput["budgetStyle"] })}
                >
                ...
                <div className="list">
                  {flightResults.map((r) => (
                    <div className="item" key={r.id}>
                      ...
                      <details>
                        <summary>Agent Reasoning Trace (per option)</summary>
            ...
            <>
              <div className="cardHeader">
                <div>
                  ...
                  <div className="subtitle">Day-wise itinerary with constraints and full agent trace.</div>
                </div>
              </div>

              {!itinerary ? (
                <div className="small">
                  Choose a city, days, budget style and interests. Then click <b>Generate Itinerary</b>.
                </div>
              ) : (
                <>
                  <div className="kpiRow">
      ...
      <div style={{ marginTop: 14 }} className="small">
        Resume note: This project demonstrates Orchestrator–Worker multi-agent design, tool-like function calls, traceability, and human-in-the-loop governance—deployed publicly on GitHub Pages.
      </div>
    </div>
  );
}